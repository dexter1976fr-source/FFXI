--[[
    AltPetOverlay - Version finale avec vraies barres graphiques
    Texte qui marche + barres windower.prim bien positionnées
]]

_addon.name = 'AltPetOverlay'
_addon.author = 'Dexter'
_addon.version = '1.0.0-final'
...
-- Create HP bar with primitives (plus fiable)
function create_hp_bar(owner, index)
    local base_y = settings.pos.y + 8 + 30 + (index * 70)
    local base_x = settings.pos.x + 8
    ...
    windower.prim.set_position(bg_id, base_x, base_y)
    ...
    windower.prim.set_position(bar_id, base_x, base_y)
...
-- Update HP bar
function update_hp_bar(bars, hp, max_hp)
    ...
    -- Update bar size
    windower.prim.set_size(bars.bar_id, bar_width, settings.bar_height)
    ...
    windower.prim.set_color(bars.bar_id, color.a, color.r, color.g, color.b)
end

-- Delete HP bar
function delete_hp_bar(bars)
    if bars then
        if bars.bg_id then windower.prim.delete(bars.bg_id) end
        if bars.bar_id then windower.prim.delete(bars.bar_id) end
    end
end
...
function update_display()
    if not settings.visible then
        display:hide()
        for owner, bars in pairs(pet_bars) do
            delete_hp_bar(bars)
        end
        pet_bars = {}
        return
    ...
    for owner, pet_data in pairs(pets) do
        ...
        -- Espace pour la barre (ligne vide)
        table.insert(lines, '')
        ...
        table.insert(lines, '')  -- Ligne vide entre pets
        index = index + 1
    end
...
-- Initialize
update_display()
windower.add_to_chat(122, '[AltPetOverlay] Final version loaded (text + graphical bars)')
