...
const AltController: React.FC<AltControllerProps> = ({ altId, altName }) => {
  ...
  const lastMainEngagedStateRef = React.useRef(false);
  ...
  // 🆕 Auto Engage: Surveiller TOUS les ALTs et attaquer quand quelqu'un engage
  useEffect(() => {
    if (!autoEngage || !altData) {
      // Reset l'état quand on désactive
      lastMainEngagedStateRef.current = false;
      return;
    }
    
    console.log(`[Auto Engage] ✅ Activated for ${altData.alt_name}`);
    
    let isActive = true;
    let hasLoggedStatus = false; // Pour éviter le spam du premier check
    
    const checkAndAttack = async () => {
      if (!isActive) return;
      
      try {
        // Récupérer TOUS les ALTs connectés
        const allAlts = await backendService.fetchAllAlts();
        
        if (!allAlts || allAlts.length === 0 || !isActive) {
          return;
        }
        
        // Trouver le premier ALT qui est engagé (et qui n'est pas nous-même)
        const engagedAlt = allAlts.find(alt => 
          alt.name !== altData.alt_name && 
          alt.is_engaged === true
        );
        
        const wasEngaged = lastMainEngagedStateRef.current;
        const isNowEngaged = !!engagedAlt;
        
        // Log unique au premier check pour confirmer que ça fonctionne
        if (!hasLoggedStatus) {
          console.log(`[Auto Engage] 👀 Monitoring ${allAlts.length} ALT(s)...`);
          hasLoggedStatus = true;
        }
        
        // 🔇 Afficher uniquement les changements d'état
        if (isNowEngaged !== wasEngaged) {
          if (isNowEngaged && engagedAlt) {
            console.log(`[Auto Engage] 🎯 ${engagedAlt.name} engaged! ${altData.alt_name} attacking...`);
            await sendCommand(`/assist ${engagedAlt.name}`);
            await new Promise(resolve => setTimeout(resolve, 1000));
            await sendCommand("/attack <t>");
          } else {
            console.log(`[Auto Engage] ✅ Combat terminé (${altData.alt_name})`);
          }
        }
        
        // Mettre à jour l'état
        lastMainEngagedStateRef.current = isNowEngaged;
      } catch (error) {
    ...
    const interval = setInterval(checkAndAttack, 2000);
  ...
  }, [altData?.alt_name, autoEngage]); // Dépendances minimales
  ...
  }, [altData?.pet_name, altData?.weapon_type, altData?.party?.length]);
  ...
  const applyConfigurationWithData = (config: any) => {
    ...
    console.log(`[AltController ${altName}] Applied config:`, {
      ...
      petAttacks: altData.pet_name ? Object.keys(altData.pet_attacks).length : 0,
            ...
            {altData.pet_name && (
              <div className="mt-1">
                <div className="flex items-center gap-2 text-xs">
                  ...
                  {altData.pet_hpp !== undefined && (
                    <span className={`font-semibold ${altData.pet_hpp < 50 ? 'text-red-400' : 'text-green-400'}`}>
                      HP: {altData.pet_hpp}%
                    </span>
                  )}
                  {/* 🆕 Charges BST Ready avec barres visuelles */}
                  {altData.main_job === 'BST' && altData.bst_ready_charges !== undefined && (
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex gap-1">
                        {[1, 2, 3].map((charge) => (
                          <div
                            key={charge}
                            className={`w-6 h-3 rounded-sm border border-yellow-600 transition-all ${
                              charge <= (altData.bst_ready_charges || 0)
                ...
                {altData.pet_hpp !== undefined && (
                  <div className="flex items-center gap-2 mt-1">
                    <div className="flex-1 bg-slate-900 rounded-full h-2 overflow-hidden">
                      <div 
                        className={`h-full transition-all ${altData.pet_hpp < 50 ? 'bg-red-500' : 'bg-green-500'}`}
                        style={{ width: `${altData.pet_hpp}%` }}
                      />
                    </div>
                  </div>
                )}
