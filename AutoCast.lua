...
function autocast.start(config_json)
    ...
    -- Démarrer le module du job si fonction start existe
    local job_module = autocast.job_modules[autocast.current_job]
    if job_module and job_module.start then
        print('[AutoCast] 🐛 Calling job_module.start()')
        job_module.start()
    end
...
function autocast.stop()
    ...
    if autocast.current_job and autocast.job_modules[autocast.current_job] then
        local job_module = autocast.job_modules[autocast.current_job]
        if job_module.cleanup then
            job_module.cleanup()
        end
    end
