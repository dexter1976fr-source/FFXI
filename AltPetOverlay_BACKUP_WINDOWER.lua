--[[
    AltPetOverlay - Version socket temps r�el
]]
...
function process_pet_data(data)
    ...
    if owner and name then
        ...
        pet_slots[slot] = {
            ...
            job = job or 'UNKNOWN',  -- Job du propriétaire
...
function build_pet_text(owner, pet_data)
    ...
    -- Utiliser le job pour déterminer quel type d'info afficher
    ...
    elseif job == 'BST' and pet_data.charges then
        ...
        for i = 1, 3 do
            if i <= pet_data.charges then
                str = str .. '\\cs(255,220,100)●\\cr'  -- Point jaune pour charge disponible
            else
                str = str .. '\\cs(80,80,80)○\\cr'     -- Point gris pour charge vide
            end
