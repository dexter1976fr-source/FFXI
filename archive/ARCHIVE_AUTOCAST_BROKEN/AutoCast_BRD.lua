...
function brd.update_songs(cfg, player)
    ...
    -- Le serveur Python décide quand caster via force_cast_mages/melees
    if brd.cycle_phase == "idle" then
        -- Rester en idle, attendre commande du serveur
        return
        
    elseif brd.cycle_phase == "mages" then
        print('[BRD DEBUG] In mages phase, song_index='..brd.cycle_song_index)
        local songs = cfg.mage_songs or {}
        print('[BRD DEBUG] Songs: '..table.concat(songs, ', '))
        if brd.cycle_song_index <= #songs then
            ...
            -- Tous les songs mages castés, retour idle
            print('[BRD AutoCast] ✅ Mages done, idle')
            brd.cycle_phase = "idle"
            brd.temp_target = nil
...
-- Force le cast des songs mages (appelé par le serveur Python)
function brd.force_cast_mages()
    print('[BRD AutoCast] 🎵 FORCE cast mages')
    brd.cycle_phase = "mages"
    brd.cycle_song_index = 1
    brd.cycle_phase_start = os.clock()
    brd.temp_target = nil  -- Retour au healer
end

-- Force le cast des songs melees (appelé par le serveur Python)
function brd.force_cast_melees()
    print('[BRD AutoCast] 🎵 FORCE cast melees')
    brd.cycle_phase = "melee"
    brd.cycle_song_index = 1
    brd.cycle_phase_start = os.clock()
    
    -- Trouver le melee et se déplacer vers lui
    local melee = brd.find_melee_target()
    if melee then
        brd.temp_target = melee
        print('[BRD AutoCast] 🎯 Moving to: '..melee.name)
    else
        print('[BRD AutoCast] ⚠️ No melee found, staying at healer')
    end
end

-- Charge la config depuis le fichier JSON
function brd.load_config_from_file()
    ...
    -- Extraire healerTarget
    local healer = content:match('"healerTarget"%s*:%s*"([^"]+)"')
    if healer and healer ~= "[Heal]" then
        brd.home_target_name = healer
    end
    ...
    if mage_songs_str then
        ...
        for song in mage_songs_str:gmatch('"([^"]+)"') do
            table.insert(mage_songs, song)
        end
        if #mage_songs > 0 then
            brd.default_config.mage_songs = mage_songs
        end
    end
    ...
    if melee_songs_str then
        ...
        for song in melee_songs_str:gmatch('"([^"]+)"') do
            table.insert(melee_songs, song)
        end
        if #melee_songs > 0 then
            brd.default_config.melee_songs = melee_songs
        end
    end
