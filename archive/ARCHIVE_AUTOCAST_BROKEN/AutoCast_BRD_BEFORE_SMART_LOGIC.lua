...
-- Vérifie si un membre du party a un buff spécifique
function brd.member_has_buff(member_name, buff_name)
    ...
    for _, buff_id in ipairs(buffs) do
        ...
        if buff_data and buff_data.en and buff_data.en:lower():find(buff_name:lower()) then
            return true
        end
    end
...
-- Vérifie si tous les membres d'un groupe ont un buff
function brd.all_members_have_buff(members, buff_name)
    for _, member_name in ipairs(members) do
        if not brd.member_has_buff(member_name, buff_name) then
            return false
        end
    end
    return true
...
function brd.update_movement(cfg, player)
    ...
    -- Ne pas bouger pendant la phase debuff (mais on a déjà casté le pending_cast au-dessus)
    if brd.cycle_phase == "debuff" then
        return
    end
...
function brd.update_songs(cfg, player)
    ...
    -- Ne pas appliquer le cooldown pendant la phase debuff (pour laisser le cast finir)
    if time_since_cast < brd.cycle_cooldown and brd.cycle_phase ~= "debuff" then
        return
    end
    ...
    elseif brd.cycle_phase == "melee" then
        ...
        if brd.cycle_song_index <= #songs then
            ...
            if cfg.use_debuffs then
                ...
                brd.cycle_song_index = 1
                brd.cycle_phase_start = os.clock()
                -- Ne pas stocker temp_target pour les debuffs, on cast directement sur <t>
            else
                print('[BRD AutoCast] 🎵 Cycle terminé (debuffs désactivés)')
                brd.cycle_phase = "idle"
                brd.temp_target = nil
                brd.cycle_song_index = 1
            end
    ...
    elseif brd.cycle_phase == "debuff" then
        ...
        if brd.cycle_song_index <= #songs then
            ...
            if not brd.pending_cast then
                -- Faire l'assist d'abord
                windower.send_command('input /assist <p1>')
                
                -- Mettre le cast en pending (sera exécuté quand on s'arrête)
                brd.pending_cast = {
                    song = song,
                    target = '<bt>',
                    queued_at = os.clock()
                }
                print('[BRD AutoCast] 🎵 Assist <p1> → Queued: '..song)
                
                brd.cycle_song_index = brd.cycle_song_index + 1
                brd.cycle_last_cast = os.clock()
            end
        else
            -- Vérifier qu'on n'a plus de cast en attente avant de retourner
            if not brd.pending_cast then
                local time_since_cast = os.clock() - brd.cycle_last_cast
                -- Attendre 7 secondes pour être sûr que le cast est fini
                if time_since_cast > 7 then
                    print('[BRD AutoCast] 🎵 Cycle terminé - Retour healer')
                    brd.cycle_phase = "idle"
                    brd.temp_target = nil
                    brd.cycle_song_index = 1
                    
                    -- Retarget le healer maintenant
                    if brd.home_target_name then
                        windower.send_command('input /target '..brd.home_target_name)
                    end
                end
            end
        end
