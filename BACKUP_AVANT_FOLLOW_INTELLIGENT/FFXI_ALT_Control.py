        ...
        try:
            ...
            if waiting_for_buffs:
                ...
                if current_phase == "mage" and healer in alts:
                    ...
                    print(f"[BRD Manager] 🐛 Mage: buff_count={buff_count}, songs_cast={songs_cast}")
                    ...
                    print(f"[BRD Manager] 🐛 Melee: buff_count={buff_count}, songs_cast={songs_cast}")
            ...
            if current_phase == "mage" and healer in alts:
                ...
                if buff_count < 2:
                    print(f"[BRD Manager] 🎵 Mage buffs missing ({buff_count}/2)")
            ...
            if current_phase == "melee" and melee in alts:
                ...
                if buff_count < 2:
                    print(f"[BRD Manager] ⚔️ Melee buffs missing ({buff_count}/2)")
                    # Follow avec distance de 2 yalms
                    send_command_to_alt(brd_name, f'//ac follow {melee} 2')
                    
                    # Attendre 1 seconde que le follow démarre
                    time.sleep(1)
                    
                    # Attendre que le BRD s'arrête de bouger
                    print(f"[BRD Manager] ⏳ Waiting for BRD to stop moving...")
                    for i in range(20):  # Max 10 secondes
                        time.sleep(0.5)
                        brd_data = alts.get(brd_name, {})
                        if not brd_data.get("is_moving"):
                            print(f"[BRD Manager] ✅ BRD stopped, casting songs")
                            break
                    
                    # Attendre encore 0.5s pour être sûr
                    time.sleep(0.5)
