...
class ServerControlGUI:
    ...
    def toggle_servers(self):
        ...
        if not servers_running:
            ...
            flask_thread.start()
            brd_thread.start()
            print("[BRD Manager] Thread started")
...
# ============ BRD INTELLIGENT MANAGER ============

def run_brd_manager_loop(stop_event):
    """Thread qui exécute le BRD manager en boucle"""
    import time
    print("[BRD Manager] Loop started")
    while not stop_event.is_set():
        try:
            brd_intelligent_manager()
        except Exception as e:
            print(f"[BRD Manager] Error: {e}")
        time.sleep(1)  # Vérifier toutes les secondes
    print("[BRD Manager] Loop stopped")

brd_last_check = 0
brd_check_interval = 5
brd_last_mage_cast = 0
brd_last_melee_cast = 0
brd_cast_cooldown = 20
brd_next_check = "mages"

def extract_song_keywords(song_name):
    """Extrait les mots-clés d'un song pour la détection"""
    keywords = []
    if "Ballad" in song_name:
        keywords.append("Ballad")
    if "March" in song_name:
        keywords.append("March")
    if "Minuet" in song_name:
        keywords.append("Minuet")
    if "Madrigal" in song_name:
        keywords.append("Madrigal")
    if "Paeon" in song_name:
        keywords.append("Paeon")
    if "Mambo" in song_name:
        keywords.append("Mambo")
    return keywords

def load_brd_config_for_check():
    """Charge la config BRD pour savoir quels songs vérifier"""
    try:
        config_file = os.path.join("A:/Jeux/PlayOnline/Windower4/addons/AltControl/data", "autocast_config.json")
        if os.path.exists(config_file):
            with open(config_file, 'r', encoding='utf-8') as f:
                config = json.load(f)
            return config.get("BRD", {})
    except:
        pass
    
    # Config par défaut
    return {
        "mageSongs": ["Mage's Ballad II", "Mage's Ballad III"],
        "meleeSongs": ["Valor Minuet V", "Sword Madrigal"]
    }

def brd_intelligent_manager():
    """Vérifie les buffs et envoie commandes de cast au BRD"""
    global brd_last_check, brd_last_mage_cast, brd_last_melee_cast, brd_next_check
    
    import time
    current_time = time.time()
    
    if current_time - brd_last_check < brd_check_interval:
        return
    
    brd_last_check = current_time
    ...
    # Charger la config pour savoir quels songs vérifier
    brd_config = load_brd_config_for_check()
    mage_songs = brd_config.get("mageSongs", [])
    melee_songs = brd_config.get("meleeSongs", [])
    
    # Extraire les keywords des songs configurés
    mage_keywords = set()
    for song in mage_songs:
        mage_keywords.update(extract_song_keywords(song))
    
    melee_keywords = set()
    for song in melee_songs:
        melee_keywords.update(extract_song_keywords(song))
    
    # Vérifier si occupé
    if current_time - max(brd_last_mage_cast, brd_last_melee_cast) < brd_cast_cooldown:
        return
    
    # Vérifier si quelqu'un est engagé
    someone_engaged = any(data.get("is_engaged") for data in alts.values())
    ...
    healer_name = None
    for name, data in alts.items():
        if name != brd_name and data.get("main_job") in ['WHM', 'SCH', 'RDM', 'GEO']:
            healer_name = name
            break
    ...
    # Alterner entre mages et melees
    if brd_next_check == "mages":
        healer_buffs = alts[healer_name].get("active_buffs", [])
        
        # Vérifier si tous les keywords des mage songs sont présents
        missing_keywords = []
        for keyword in mage_keywords:
            if not any(keyword in buff for buff in healer_buffs):
                missing_keywords.append(keyword)
        
        print(f"[BRD Manager] {healer_name} buffs: {healer_buffs} | Need: {mage_keywords} | Missing: {missing_keywords}")
        
        if missing_keywords:
            if current_time - brd_last_mage_cast > brd_cast_cooldown:
                print(f"[BRD Manager] {healer_name} missing mage buffs ({missing_keywords}), casting {mage_songs}...")
                send_command_to_alt(brd_name, "//ac cast_mage_songs")
                brd_last_mage_cast = current_time
        ...
        melee_name = None
        for name, data in alts.items():
            if name != brd_name and data.get("is_engaged"):
                melee_name = name
                break
        
        if melee_name:
            melee_buffs = alts[melee_name].get("active_buffs", [])
            
            # Vérifier si tous les keywords des melee songs sont présents
            missing_keywords = []
            for keyword in melee_keywords:
                if not any(keyword in buff for buff in melee_buffs):
                    missing_keywords.append(keyword)
            
            print(f"[BRD Manager] {melee_name} buffs: {melee_buffs} | Need: {melee_keywords} | Missing: {missing_keywords}")
            
            if missing_keywords:
                if current_time - brd_last_melee_cast > brd_cast_cooldown:
                    print(f"[BRD Manager] {melee_name} missing melee buffs ({missing_keywords}), casting {melee_songs}...")
                    send_command_to_alt(brd_name, "//ac cast_melee_songs")
                    brd_last_melee_cast = current_time
                    brd_next_check = "mages"
                    return
